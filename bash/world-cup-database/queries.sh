...
echo "$($PSQL "SELECT t.name FROM games g JOIN teams t ON t.team_id = g.winner_id WHERE g.year = 2014 AND g.round = 'Eighth-Final' UNION SELECT t.name FROM games g JOIN teams t ON t.team_id = g.opponent_id WHERE g.year = 2014 AND g.round = 'Eighth-Final' ORDER BY name;")"
